# Loading Libraries}
    ...
    library(dplyr)
    library(ggplot2)
    library(ggfortify)             # Load ggfortify
    library(DESeq2)
    library(gridExtra)
    library(ggrepel)
...
# Pre-processing ----------------------------------------------------------
    ...
    #   mtx.agg[mtx.agg==0|0.0|0.00|0.000|0.0000|0.00000|0.0000000|0.00000000|0.0000000] <- 0
    varrow <- apply(mtx.agg, 1, var, na.rm=TRUE)
    cons_var <- (varrow == 0 | is.na(varrow))
    dim(mtx.agg)                        #   35309    28
    mtx.agg.var <- mtx.agg[!cons_var,]
    dim(mtx.agg.var)                    #   20457    28
...
# DESeq2 ------------------------------------------------------------------
    ...
    res.dems2=res.df[res.df$padj< 0.01 & abs(res.df$log2FoldChange) >3,]
    res.dems2=res.dems2[order(res.dems2$log2FoldChange), ]
...
# Volcano plot ------------------------------------------------------------
    ...
    ggplot(res_df, aes(x = log2FoldChange, y = -log(padj), col = res_df$diffexpressed),label=delabel) +
        ...
        ggtitle('TNBC vs Controls Volcano Plot')+  # Plot title 
...
#object <- list(DAVID_MF=DAVID_MF, Res.dems=Res.dems)
    circ <- circle_dat(DAVID, Res.dems)
## Generate a simple barplot
    GOBar(subset(circ, category == 'GOTERM_MF_DIRECT'), title = 'GOTERM_MF_DIRECT')
    GOBar(circ, display = 'multiple', title = 'Z-score Coloured Barplot', zsc.col = c('limegreen', 'white', 'navyblue'))
# Generate the bubble plot with a label threshold of 3
    GOBubble(circ, labels = 4,ID=T,table.col=T)
# Add a title, change the colour of the circles, facet the plot according to the categories and change the label threshold
    GOBubble(circ, title = 'Bubble plot', colour = c('orange', 'darkred', 'gold'), bg.col = F,display = 'multiple', labels = 3,ID=T)   
